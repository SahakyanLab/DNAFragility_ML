    ...
    private = list(
        ...
        get_ref_seq = function(assembly){
            private$ref <- switch(assembly,
                ...
                "telo_to_telo" = BSgenome.Hsapiens.NCBI.T2T.CHM13v2.0::BSgenome.Hsapiens.NCBI.T2T.CHM13v2.0
            )

            if(!any(grepl(pattern = "^chr", x = seqnames(private$ref)))){
                chr_names <- paste0("chr", seqnames(private$ref))
                seqnames(private$ref@seqinfo) <- seqnames(private$ref) <- chr_names
            }
        ...
        get_true_breaks = function(){
            ...
            to.keep <- lapply(paste0("chr", private$which_chr), function(chrs){
