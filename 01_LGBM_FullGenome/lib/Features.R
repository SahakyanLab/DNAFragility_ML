    ...
    public = list(
        ...
        get_features = function(break_type = "biological", ranges = NULL,
            ...
            if(FEAT_PROCESS_LIKE_IMAGE){
                ...
                self$column_names$TFBS <- colnames(true.breaks.mat)[tfbs.ind]

                # QM parameters
                qm.ind <- which(grepl(pattern = "^[A-Z]_ds.dEhof_ds_ss", colnames(true.breaks.mat)))
                self$column_names$QM_PARAMETERS <- colnames(true.breaks.mat)[qm.ind]

                # G4seq map
                g4.ind <- which(grepl(pattern = "^G4seq", colnames(true.breaks.mat)))
                self$column_names$G4MAP <- colnames(true.breaks.mat)[g4.ind]
                ...
                self$column_names$EPIGENOME_MARKS <- colnames(true.breaks.mat)[epigenome.ind]            
                ...
                self$column_names$BREAKS <- colnames(true.breaks.mat)[breaks.ind]
                ...
                if(FEAT_G4_REGEX){
                    ...
                    self$column_names$G4_REGEX <- "G4seq_counts"
                ...
                if(FEAT_GC_COUNT){
                    ...
                    self$column_names$GC_CONTENT <- "GC_content"
                    self$column_names$GC_COUNT <- "GC_content"
            ...
            if(SAVE_OUTPUT){
                ...
                to.keep <- c("predictor", self$column_names$BREAKS)
                write_parquet(
        ...
        select_columns = function(cols = "all"){
            ...
            to.filter <- switch(cols,
                "only_breaks" = self$column_names$BREAKS,
                "only_triplets" = self$column_names$KMER_COUNTS,
                "only_singleton" = self$column_names$SINGLETON,
                "only_gc_content" = self$column_names$GC_CONTENT,
                "only_gc" = self$column_names$GC_COUNT,
                "singleton_and_gc" = unlist(c(
                    self$column_names$SINGLETON,
                    self$column_names$GC_CONTENT),
                    use.names = FALSE),
                "triplets_and_gc" = unlist(c(
                    self$column_names$KMER_COUNTS,
                    self$column_names$GC_CONTENT),
                    use.names = FALSE),                    
                "pre_parameterised" = unlist(c(
                    self$column_names$BREAKS,
                    self$column_names$QM_PARAMETERS,
                    self$column_names$TFBS,
                    self$column_names$G4MAP,
                    self$column_names$LONGRANGE_PREPARAMS
                    # self$column_names$EPIGENOME_MARKS
                ), use.names = FALSE),
            ...
            if(!length(to.filter) > 0) to.filter <- unlist(self$column_names, 
                                                           use.names = FALSE)
    ...
    private = list(
        ...
        get_longrange_preparams = function(data){
            ...
            if(data == "breaks"){
                ...
                self$column_names$LONGRANGE_PREPARAMS <- colnames(kmer.counts.all)

                # private$kmer_counts$true_breaks <- kmer.counts
                # self$column_names$KMER_COUNTS <- colnames(kmer.counts)
            } else if(data == "control"){
        ...
        get_gc_count = function(data){
            ...
            if(data == "breaks"){
                ...
                self$column_names$SINGLETON <- c("A", "C", "G", "T")
            } else if(data == "control"){
        ...
        get_kmer_counts = function(data, crash_test = FALSE){
            ...
            #     self$column_names$KMER_COUNTS <- colnames(kmer.counts.all)
            # } else {
        ...
        run_viennaRNA_fold = function(data, nuc_type, RNAfold.CALL){
            ...
            self$column_names$VIENNA_RNA <- colnames(private$viennaRNA$true_breaks)
            total.time <- Sys.time() - t1
            cat("DONE! --", signif(total.time[[1]], 2), 
                attr(total.time, "units"), "\n")
        ...
        get_dnashape = function(data){
            ...
            if(data == "breaks"){
                ...
                self$column_names$DNA_SHAPE <- colnames(private$dna_shape$true_breaks)
            } else if(data == "control"){
